...
namespace FoodieMVC.Controllers
{
    public class AdminController : Controller
    {
        ...
        [HttpGet("Admin/UserOrders/{userId}")]
        public async Task<IActionResult> UserOrders(int userId)
        {
            string endpoint = $"api/orderdetails/User/{userId}"; // Adjust if your API route differs

            var orders = await clientHelper.GetData<List<OrderDetailsDTO>>(endpoint);

            if (orders == null || orders.Count == 0)
            {
                ViewBag.Message = $"No orders found for User ID {userId}.";
